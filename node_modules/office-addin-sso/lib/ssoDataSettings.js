...
exports.writeApplicationData = exports.getSecretFromCredentialStore = exports.addSecretToCredentialStore = void 0;
...
function addSecretToCredentialStore(ssoAppName, secret, isTest = false) {
    try {
        switch (process.platform) {
            case "win32":
                ...
                child_process_1.execSync(addSecretToWindowsStoreCommand, { stdio: "pipe" });
                break;
            case "darwin":
                ...
                if (existingSecret !== '') {
                    ...
                    child_process_1.execSync(updatSecretInMacStoreCommand, { stdio: "pipe" });
                }
                else {
                    const addSecretToMacStoreCommand = `${isTest ? '' : 'sudo'} security add-generic-password -a ${os.userInfo().username} -s "${ssoAppName}" -w "${secret}"`;
                    child_process_1.execSync(addSecretToMacStoreCommand, { stdio: "pipe" });
                }
                break;
            default:
    ...
    try {
        switch (process.platform) {
            case "win32":
                ...
                return child_process_1.execSync(getSecretFromWindowsStoreCommand, { stdio: "pipe" }).toString();
            case "darwin":
                const getSecretFromMacStoreCommand = `${isTest ? "" : "sudo"} security find-generic-password -a ${os.userInfo().username} -s ${ssoAppName} -w`;
                return child_process_1.execSync(getSecretFromMacStoreCommand, { stdio: "pipe" }).toString();
                ;
            default:
                const errorMessage = `Platform not supported: ${process.platform}`;
                throw new office_addin_usage_data_1.ExpectedError(errorMessage);
    ...
    return __awaiter(this, void 0, void 0, function* () {
        try {
            if (fs.existsSync(manifestPath)) {
                ...
                yield office_addin_manifest_1.modifyManifestFile(manifestPath, 'random');
                return true;
            }
            else {
